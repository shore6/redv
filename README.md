...
- **バス(`reg[N]` / `input[N]` / `output[N]` / `var[N]`)** — 複数レーンを束ねて宣言し、
  ...
  端点には **スライス `a[hi:lo]`**(部分バス・向きでビット反転)と **連結 `{a, b}`**(各要素を連接)も
  書けます。片側がスカラなら **バス↔スカラの直結**(`x - y;` で fan-in = MAX 合流 / fan-out = 複製)も
  そのまま書けます。
...
| `examples/clock_sugar.rv` | `clock(var, N)`: テスト用クロックを 1 行で生成(各レベル N tick 保持で 0/15 を自動トグル) |
...
| `examples/bus_slice_concat.rv` | バスのスライス `a[hi:lo]`(ビット反転)と連結 `{a, b}`(左ローテート) |
| `examples/bus_scalar.rv` | バス↔スカラの直結: fan-in(`buf - y;` = MAX 合流)と fan-out(`a - y;` = 複製) |
