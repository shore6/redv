...
| `examples/const_bus.rv` | バス const reg の裸数値初期化 `const reg[W] n = <リテラル>;`:ニブル分解で比較器閾値をレーンごとに 1 定数で表す |
| `examples/bus_lane_call.rv` | レーン / スライスを logic 呼び出しの引数とタプル束縛 target に直接書くリップルキャリー加算器 |
| `examples/concat_call.rv` | 連結を呼び出しに直接書く:`g({a, b, c})` でスカラを束ね、`{rest, msb} = g(x)` で出力を分配 |
| `examples/call_arg_broadcast.rv` | 幅 1 の引数をバス入力ポートへブロードキャスト:1 本のイネーブル線でバスをマスク |
...
| `examples/stdlogic_generic.rv` | stdlogic のジェネリック幅:`s_xor#(W=4)` などで基本ゲートを 4 ビット化 |
| `examples/stdlogic_reduction.rv` | stdlogic のリダクションゲート:`s_rand#(W=4)` などでバス 1 本をスカラ 1 出力へ畳み込む |
...
| `examples/pack_unpack.rv` | 整数⇔バスのビット変換(`x = unpack(v);` / `pack(x)`)で参照モデルと全数突き合わせ |
| `examples/sim_bus_slice.rv` | sim バス var のスライス:代入 / パルス / clock / pack / unpack を部分レーンに掛ける |
