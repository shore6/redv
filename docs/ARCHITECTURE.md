...
- 原実装(C++ 版)の移植であり、**意味論は C++ 版と一致** させてある。各モジュール冒頭の `//!`
  に「C++ 版 `xxx.hpp` の移植」と対応を明記している。
...
| `parser.rs` | 中 | 再帰下降構文解析。`Token` 列 → `ast::Program`。`#include` はサブパーサで同じ `Program` を共有 |
...
| `diag.rs` | 小 | 診断。`RvError`(行番号付き)・`fail()`(Err 生成)・`warn()`(stderr) |
...
- `Token` は `{ k, s, num, line }`。`Num` は値を `num` に、`Ident`/`Str`/`Punct` はテキストを `s` に持つ。
- コメント `//`(行)・`/* */`(範囲)はトークンを生成せずスキップ。未終端ブロックコメント / 文字列はエラー。
- 2 文字演算子 `<= >= == != && ||` は先読み 1 バイトでまとめる。それ以外の記号は 1 文字 `Punct`。
- 行番号 `line` は各トークンに付き、以降の診断で使う。
...
`Parser` は `{ toks, i, base_dir }`。再帰下降で `Program` を構築する。`Program` は構造体に保持せず
**各メソッドへ `&mut` 引数で渡す**(`#include` のサブパーサが同じ `Program` を共有するため)。

トップレベルは `#`(ディレクティブ)/ `logic` / `module` の 3 種(`parse_file`)。

- **logic** (`parse_logic` / `parse_logic_stmt`): ポート列 → 本体文。本体文は `wire` 宣言・`reg` 宣言・
  チェーン文・代入/インスタンス化。
  - **文の判別**: 先頭の識別子(端点。`.side` / `[k]` が付き得る ← `parse_chain_part`)を読み、
    次が `-` なら **チェーン接続文**(`from -..- to`)、`=` なら **代入 / インスタンス化**。
  - `out = callee(args...)` は階層インスタンス化(`Instance`)。`= [strength] elem` は `AssignSingle`。
...
C++ 版は「enum タグ + 全フィールド同居の 1 構造体」だったが、Rust 版は **データ付き enum** にして
不正状態を型で表現不能にしている(素子種別・文種別の追加時にコンパイラが網羅性を検査できる)。
...
- **トップ**: `Program { defines, str_defines, logics, modules }`。
...
- `RvError { line, msg }`。`Display` は `line > 0` なら `line N: msg`、さもなくば `msg`。
- `fail(line, msg) -> RvResult<T>`: `Err` を生成(呼び出し側で `?` か `return`)。
- `warn(line, msg)`: stderr へ `[warning] ...`。

**規約**: 回復不能はすべて `fail`(= `Err` 伝播)、続行可能な注意は `warn`。`line == 0` は
「特定の行に紐付かない」エラー/警告(エラボレーション結果や sim 実行時など)を表す。
...
- **順序素子** `CSeq` — リピータ・トーチ・コンパレータ(遅延つき)。`ZeroRep` は遅延ゼロの `r0`。
...
- `d` / `dx`(ダスト): `decay` を +1 するだけ(ノードを作らず次のエッジに減衰として乗せる)。
- `b`(ブロック): `Block` ノードを作り、直前から `decay` のエッジを張る。
- `r1`–`r4` / `t`(リピータ/トーチ): 入力ノード `#i` と出力ノード `#o` を作り、`add_seq` で順序素子化。
...
`v = g(args)`(`CallBind` → `do_call_bind`)で:

- 初回はキー `g(arg1,arg2)` でインスタンスを生成・エラボレートし、引数 var を入力ノードに、出力ポートを
  out_bind に登録する。同じキーの 2 回目以降は同じインスタンスを再利用(`target` はキーに含めない =
  複数 var が同じ出力を観測できる)。
- バス var ↔ バスポートはレーン対応で束縛する。形(スカラ/バス)と幅が一致しないとエラー。
...
意味論の確定値は LANGUAGE.md §7 が一次情報。ここでは **実装の構造** を説明する。
...
## 8. 借用検査との付き合い方(Rust 移植の注意)

C++ 版が参照イテレートやポインタ操作で書いていた箇所は、Rust では借用検査に通すために形を変えてある。
**結果は原実装と同一**:
...
| 順序素子(`CSeq`) | 遅延を持つ素子(リピータ/トーチ/コンパレータ)。履歴 `hist` を持つ |
