...
- **logic** (`parse_logic` / `parse_logic_stmt`): ポート列 → 本体文。本体文は `wire` 宣言・`reg` 宣言・
  ...
  - `out = callee(args...)` および `(o1, o2, ...) = callee(args...)` は階層インスタンス化(`Instance`)。
    ...
    レーン / スライス添字が付き得る(issue #118)。`= [strength] elem` は `AssignSingle`。
    `target = a-b-c`(`=` ありで複数チャンク)は wire への素子列定義(`AssignChain`)。
    添字つき target は callee 呼び出しが続く場合のみ合法で、素子代入では従来どおりエラー。
...
- **logic 側**: `LogicStmt`(`DeclWire` / `DeclReg` / `AssignChain` / `Chain` / `AssignSingle` /
  ...
  引数: 名前 + レーン選択 or ネスト呼び出し。`Instance` と `CallBind` で共用、issue #97)、
  `BindTarget`(束縛 target: 名前 + レーン選択 `Sel`。issue #118)。
...
- 引数と target の親側端点は `resolve_parent_ref` が `PortShape` へ解決する。レーン / スライス添字
  (`BindTarget.sel` / `InstArg::Name.sel`、issue #118)はチェーン端点と同じ `Sel` を流用し、
  `bus_lane` / `bus_slice` でレーン列を切り出すだけ(circuit 側は無変更)。
...
- 初回はキー `g(arg1,arg2)` でインスタンスを生成・エラボレートし、引数 var を入力ノードに、各出力ポートのレーン列を
  ...
  別の var 組で同じ出力を観測できる)。引数の添字はキーに含める(`sim_arg_key`、issue #118:
  `g(x[0])` と `g(x[1])` は別インスタンス。`x[k:k]` は `x[k]` へ正規化)。
...
- 出力ポートごとに、対応する target(スカラ var / バス var / そのレーン・スライス)へレーン対応で `out_bind` を登録する。
  target は `bind_target_keys` がレーンキー列へ解決し(`sel_lane_indices` を共用)、形(スカラ/バス)と
  幅が一致しないとエラー。出力ポート数と target 数の厳格一致・解決後レーンの重複も `do_call_bind` で検査する(LANGUAGE.md §5.5)。
