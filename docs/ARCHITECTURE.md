...
`-W error` 指定時は、完走後に `diag::warning_count()` を見て警告が 1 件でもあれば
終了コード 1 にする(issue #48。§3.6)。
...
- `Num` は 10 進、`0b...`(2 進)、`0x...`(16 進)の 3 形式を同じ `Tk::Num` に落とす。接頭辞直後に有効な数字が無いとき(`0b` 単独や `0xg` 等)は誤記としてエラーにする(かつては強度ブロック `0b` を温存するため `0` + 後続に分割していたが、ブロック素子の廃止(issue #75)で分割の理由がなくなった)。
...
- **logic** (`parse_logic` / `parse_logic_stmt`): ポート列 → 本体文。本体文は `wire` 宣言・`reg` 宣言・
  ...
  - `out = callee(args...)` および `(o1, o2, ...) = callee(args...)` は階層インスタンス化(`Instance`)。
    parser はどちらも `Instance.outputs: Vec<BindTarget>` に正規化する。引数と target には
    レーン / スライス添字(issue #118)と連結 `{...}`(issue #123、`parse_concat_refs`)が
    付き得る。`= [strength] elem` は `AssignSingle`。
    `target = a-b-c`(`=` ありで複数チャンク)は wire への素子列定義(`AssignChain`)。
    添字つき / 連結の target は callee 呼び出しが続く場合のみ合法で、素子代入では従来どおりエラー。
- **module** (`parse_module` / `parse_sim_stmt`): `var` 宣言と `sim { ... }` ブロック。
  - sim 文は `#n`/`#init`、`?monitor`、`var`、`if`/`while`/`for`、呼び出し `f(...)`、代入 `x = ...`、
    ...
    従来エラーだったので純加算で割り当てる。`x[k] = callee(args)` / `x[hi:lo] = callee(args)` は
    レーン / スライスへの束縛(issue #118)で、`=` の後を先読みして通常のレーン代入と判別する
    (束縛の添字は定数式、レーン代入の添字は実行時式)。文頭 `{` は連結 target の束縛
    `{c, s[3:0]} = callee(args)`(issue #123。従来エラーだった列に純加算)。
...
- **logic 側**: `LogicStmt`(`DeclWire` / `DeclReg` / `AssignChain` / `Chain` / `AssignSingle` /
  ...
  `RegInit`(reg 初期化子)、`Qual`(`Plain`/`Const`。`Mutable` は issue #142 で廃止)、`InstArg`(logic 呼び出しの
  引数: 名前 + レーン選択 or 連結 or ネスト呼び出し。`Instance` と `CallBind` で共用、issue #97 / #123)、
  `BindRef`(名前 + レーン選択 `Sel` の単純参照)、`BindTarget`(束縛 target: `Ref` or
  連結 `Concat(Vec<BindRef>)`。issue #118 / #123)。
- **module/sim 側**: `SimStmt`(`DeclVar` / `Assign` / `Unpack` / `CallBind` / `WaitTicks` / `WaitInit` /
  `Call` / `MonReg` / `If` / `While` / `For`)、`Expr`(`Num`/`Var`/`Time`/`Bin`/`Un`/`Pack`)、`CallData`。
- **トップ**: `Program { defines, logics, modules, included_stdlibs }`。`defines` には数値 `#define` に加え
...
- `lint(line, rule, msg)`: stderr へ `[lint] rule: ...`(issue #48)。実行時警告と混ざらない
  lint 専用種別で、JSON モードでは `{"kind":"lint","rule":"...","line":N,"msg":"..."}` になる。
- `warning_count()`: これまでに出した警告(`warn` / `lint`)の総数。`AtomicU64` のグローバル
  カウンタで、`-W error` 指定時に main が完走後に見て終了コードを決める(警告即 abort に
  しないのは、エラボレーションが sim 実行中に遅延発生するため。§4.7)。
...
- **ノード** `CNode` — reg / ポート / 素子の入出力点。種別 `NodeKind`(`Plain`/`Const`/`Input`)。
...
- 併合時に種別の **ランク**(`Const` > `Input` > `Plain`)が高い方を残し、`has_incoming` /
...
- `r1`–`r4` / `t` / `o`(リピータ/トーチ/オブザーバ): 入力ノード `#i` と出力ノード `#o` を作り、
  ...
  エッジ亜種 `op` / `on` / `oe` は `parse_chunk` が `Elem.n` にモードを載せ(0=変化全部, 1=立ち上がり,
  2=立ち下がり, 3=2値エッジ)、`SeqKind::Observer(ObsMode)` のペイロードに変換される。
...
- 引数と target の親側端点は `resolve_parent_ref` が `PortShape` へ解決する。レーン / スライス添字
  ...
  各要素を同じ経路で解決してレーン列を連接し、幅 = 要素和のバス `PortShape` として扱う。
- **再帰インスタンス化**(自己・相互の循環)は `stack` で検出してエラー。出力ポート数と target 数は厳格一致(過不足ともエラー)、未接続 output ポートはエラー。
- target の重複は **解決後レーン単位** で検査する(`check_target_overlap`。logic 本体はノード id、
  sim は var レーンキーで比較し、`(sum[1:0], sum[1])` のような部分重複も検出する。issue #118。
  連結 target 内部の重複 `{p, p}` も同じヘルパーで検出する。issue #123)。
  字面が同一の重複はパース時にも早期エラーになる(`parse_target_tuple`)。
- 引数は `InstArg`(名前 + レーン選択 or **ネスト呼び出し**、issue #97)。ネスト呼び出しは
  `resolve_inst_arg` がサブ logic を新規インスタンスとして再帰展開し、その単一出力ポートの
...
レーン / スライス添字(`Sel` の `IdxExpr`、issue #89)も同じ二相解決に乗る。parse 時に
logic param を含まない式は `IdxExpr::Lit` へ即時簡約し、含む式は `IdxExpr::Expr` のまま遅延して
`resolve_ref()` 内の `resolve_idx(ie, &param_env, &defines)` がチェーン展開時に評価する。
範囲検査(`bus_lane` / `bus_slice`)は評価後の i32 に対して従来どおり働く。
...
`elaborate()` の末尾(未接続 output のエラー検査の後)で、動きはするが疑わしい構造を
`diag::lint()` で警告する。ルールと出力形式は [LANGUAGE.md §10.4](LANGUAGE.md) を参照。
実装は 2 系統に分かれる。

**静的ルール**(floating-reg / unused-wire / unused-input)は宣言と結線の形だけを見る。
判定の材料はエラボレーション中に収集する:

- `CNode.has_outgoing` — そのノードがどこかを駆動しているか。`add_edge` の源、順序素子の
  後ろ / 横入力、`r0` の入力で立て、`merge()` は他フラグと同じく OR 合成する
  (`has_incoming` の対になるフラグ)。
- `used_wires` — チェーン展開(`expand_chain_tokens`)で実際に参照した wire 名。循環検出用の
  `visited` は push/pop で消えるため、別の集合に積む。
- `decl_lines` — reg / バス reg / wire の宣言行(警告の報告位置)。

入力ポートと併合された reg(`z = a;`)は代表ノードの `kind == Input` で floating-reg から
除外し、unused-input 側で 1 回だけ拾う。別名併合で同じ点になった複数の reg は代表ノードで
重複排除する。同じ logic を複数回インスタンス化しても宣言由来の警告が重複しないよう、
`ModuleExec` が `linted_logics: HashSet<logic名>` を持ち、`Elaborator` が借用して 1 回に抑える。

**上界解析ルール**(always-on-torch / unreachable-output)は `Circuit::lint_potentials()` で
判定する。`step()` と同じ MAX 合流の不動点を「値」ではなく「上界」で回す: 入力 = 15、
const = 宣言値を源に、リピータ / オブザーバは `後ろ上界 > 0 ? 15 : 0`、トーチは常に 15
(履歴が 0 始まりなので必ず一度は点灯する)、コンパレータは後ろ上界(横入力は減らす方向に
しか効かない)で上へ伝える。単調なので必ず収束し、**上界 0 = そのノードは絶対に >0 に
ならない** が保証される(誤検出なし)。

上界解析は **トップレベルの elaborate でのみ** 実行する。サブインスタンスの入力ポートは
親からの結線(`connect_ports`)がサブの `elaborate` 完了後に行われるため、途中で解析すると
全ノードが上界 0 に見えて誤検出する。報告対象はこの elaborate で新規生成した範囲
(`seq_lo` 以降の順序素子と自分の出力ポート)に限り、インスタンスキャッシュが効かない
別引数の再インスタンス化でも過去分を重複報告しない。トーチの報告位置には `CSeq.line`
(素子を生成したソース行)を使う。
...
`(t1, t2, ...) = g(args)`(`CallBind` → `do_call_bind` → `ensure_instance`)で:
...
- 初回はキー `g(arg1,arg2)` でインスタンスを生成・エラボレートし、引数 var を入力ノードに、各出力ポートのレーン列を
  ...
  別の var 組で同じ出力を観測できる)。引数の添字と連結はキーに含める(`sim_arg_key`、issue #118 / #123:
  `g(x[0])` と `g(x[1])` は別インスタンス。`x[k:k]` は `x[k]`、単一要素の連結 `{x}` は `x` へ正規化)。
- **ノード名プレフィックスはキーと分ける**(issue #101)。ジェネリック呼び出しのキーは `#(...)` を含む
  (`inv#(W=2)(a)`)が、名前の `#` はトレース / VCD 非表示の印(§9)なので、キーのまま使うと
  ジェネリックインスタンスの全ノードが観測不能になる。キー中の `#` は `#(` の形でしか現れない
  (引数キーは識別子かネストインスタンスキー)ため、`#(` → `(` の置換(`inv(W=2)(a)`)をプレフィックスに使う。
...
- 出力ポートごとに、対応する target(スカラ var / バス var / そのレーン・スライス / 連結)へレーン対応で `out_bind` を登録する。
  target は `bind_target_keys` がレーンキー列へ解決し(`sel_lane_indices` を共用。連結は要素の
  キー列を連接)、形(スカラ/バス)と幅が一致しないとエラー。出力ポート数と target 数の厳格一致・解決後レーンの重複も `do_call_bind` で検査する(LANGUAGE.md §5.5)。
...
`SimStmt::Unpack` に落とす(タプル束縛経由で `CallBind` に残った `unpack` は interp が弾く)。
逆変換の `pack(x)` は式 builtin(`Expr::Pack`)で、`parse_primary` が「ident `pack` + `(`」
(従来は構文エラーのトークン列)にだけ割り当てるため、`pack` という名前の var は従来どおり使える。
...
- `Input` — sim 変数で駆動される入力(top-level 入力ポート)。値は毎 tick 先頭で `base`(var 駆動値)
  から再設定され、エッジ寄与は `Plain` と同じく max で合流する(= 実効値は max(var 駆動, 回路内配線)。
  issue #99。階層インスタンスの入力ポート `Plain` とセマンティクスが揃う)。
...
**オブザーバ** `Observer(ObsMode)` も `(back, side)` の純関数では決まらない(隣接 2 サンプル `in(T-2)` と
`in(T-1)` の比較)ので、`seq_out_of` ではなく `observer_out(mode, prev, cur)` を **phase 1 / phase 4 の両方** で
`hist` の前後から直接呼ぶ(履歴 2 段に乗るのでロックと同じく両相を対応させる)。
`ObsMode`(変化全部 / 立ち上がり / 立ち下がり / 2値エッジ)は判定式だけを差し替え、履歴と両相の扱いは共通。
...
- 端点解決: `Ep::Single`(スカラ点)/ `Ep::Bus`(レーン列)。バスチェーンは両端同幅必須で element-wise に展開。
  レーン / スライス添字は定数式(`IdxExpr`)で、ジェネリック param を含む式のみインスタンス化時に評価する(§4.6)。
...
**合流は単調 MAX 更新**(`Plain` は max)なので不動点は決定的に収束し、
...
- **ノード名の `#`**: 内部ノード(`foo.w#i3` 等)は `dump_trace` でスキップされる。`-t` で公開したい点は
  名前に `#` を含めない。ジェネリックインスタンスのノード名は `#(...)` 由来の `#` を除いた形
  (`inv(W=2)(a).x[0]`)で生成される(§5.3、issue #101)。`--vcd <file>` の VCD 波形出力
  (`circuit.rs::dump_vcd`、`Vcd`)も **同じ公開ノード基準** で信号を選び、`dump_trace` と並んで
  `step()` から毎 tick 駆動される。値変化のみを記録し、時刻は生 tick(`#init` 整定も含む)。
  VCD ゴールデンは生成ファイルを `tests/expected/*.vcd` と比較する(`run_vcd_golden`)。
